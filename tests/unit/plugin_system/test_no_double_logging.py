...
from d_post.plugin_system import PluginLoader
...
def test_lazy_load_device_does_not_relog_pc_registration(caplog):
    ...
    caplog.set_level(logging.DEBUG, logger="d_post.plugin_system")
    ...
    pc_logs = [
        ...
        if "Pc plugin 'test_pc' registered via" in r.getMessage()
    ]
    assert (
        len(pc_logs) == 1
    ), "PC plugin registration should not be re-logged during device lazy load"
