...
def base_device_config() -> DeviceConfig:
    return DeviceConfig(
        ...
        watcher=WatcherSettings(
            poll_seconds=0.1, stable_cycles=2, max_wait_seconds=5.0
        ),
    )
...
def test_matches_file_by_extension(
    base_device_config: DeviceConfig, filename: str, expected: bool
) -> None:
...
def test_matches_directory_by_contents(
    tmp_path, directory_matching_config: DeviceConfig
) -> None:
...
def test_matches_directory_requires_expected_contents(
    tmp_path, directory_matching_config: DeviceConfig
) -> None:
    folder = tmp_path / "bundle"
    folder.mkdir()
...
def test_matches_all_extensions_when_unrestricted(
    base_device_config: DeviceConfig,
) -> None:
    wildcard = replace(
        base_device_config,
