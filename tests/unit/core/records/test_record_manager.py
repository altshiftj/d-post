...
from d_post.core.records.record_manager import RecordManager
...
def record_manager(fake_sync, tmp_settings):
    with patch(
        "d_post.core.records.record_manager.load_persisted_records", return_value={}
    ):
...
def sample_record():
    return LocalRecord(
        identifier="dev-usr-inst-sample", sample_name="sample", date="20240101"
    )


def test_create_record_generates_proper_id_and_sample(record_manager):
    with patch(
        "d_post.core.records.record_manager.generate_record_id",
        return_value="dev-usr-inst-sample_a",
    ):
        record = record_manager.create_record("usr-inst-sample_A")

    assert isinstance(record, LocalRecord)
    assert record.identifier == "dev-usr-inst-sample_a"
...
def test_add_item_to_record_saves_it(tmp_path, record_manager):
    ...
    record = LocalRecord(
        identifier="dev-usr-inst-samplex", sample_name="sampleX", date="20240101"
    )
    with patch(
        "d_post.core.records.record_manager.save_persisted_records"
    ) as mock_save:
...
def test_remove_item_from_record_persists_state(tmp_path, record_manager):
    ...
    record = LocalRecord(identifier="dev-usr-inst-samplex")
    ...
    mock_save.assert_called_once()
    assert (
        str(file_path.resolve()) in record.files_uploaded
    )  # removal deferred (see TODO in implementation)


def test_get_record_by_id_case_insensitive(record_manager):
    record = LocalRecord(identifier="rem-usr-inst-sampleZ")
    record_manager._persist_records_dict = {"rem-usr-inst-samplez": record}
    assert record_manager.get_record_by_id("REM-usr-inst-SampleZ") == record
...
def test_sync_records_to_database_skips_non_inst(record_manager):
...
def test_sync_records_to_database_uploads_inst(record_manager):
    record = LocalRecord(identifier="dev-usr-inst-sample")
    record.files_uploaded = {"f": False}
    record_manager._persist_records_dict = {record.identifier: record}

    with patch("d_post.core.records.record_manager.save_persisted_records"):
        with patch.object(
...
def test_persist_records_dict_lazy_loads_once(fake_sync):
    with patch(
        "d_post.core.records.record_manager.load_persisted_records",
...
def test_sync_record_deletes_if_no_files_remain(record_manager):
    ...
    record_manager._persist_records_dict = {"dev-usr-inst-sample": record}
    ...
    assert "dev-usr-inst-sample" not in record_manager._persist_records_dict
