...
def sync_mgr(fake_ui, monkeypatch):
    import d_post.core.sync.sync_kadi as _mod
...
def test_initialize_new_db_record(sync_mgr, local_record):
    ...
    class DummyContext:
        def __init__(self):
            ...
            self.db_user = DummyKadiUser("abc-inst")
            self.device_user = DummyKadiUser("device_user")
            self.device_record = DummyKadiRecord()
            self.device_record.id = "device_record_id"
    ...
    sync_mgr._initialize_new_db_record(local_record, context)

    dummy_record.set_attribute.assert_any_call("title", local_record.sample_name)
    dummy_record.set_attribute.assert_any_call("description", None)
    dummy_record.set_attribute.assert_any_call("type", "rawdata")
    dummy_record.link_record.assert_called_with(
        context.device_record.id, "generated by"
    )
    dummy_record.add_user.assert_any_call(user_id=context.db_user.id, role_name="admin")
    dummy_record.add_group_role.assert_not_called()
...
def test_upload_record_files_returns_false_when_all_missing(sync_mgr):
    record = LocalRecord(identifier="abc-inst-sample1")
    record.files_uploaded = {
