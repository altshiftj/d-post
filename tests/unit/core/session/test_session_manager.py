...
from d_post.core.config import current
from d_post.core.records.local_record import LocalRecord


def _make_record(identifier: str = "udr_01-mus-inst-sample_a") -> LocalRecord:
...
def test_note_activity_starts_session(fake_ui, config_service):
    ...
    assert session_manager.session_active is True
    assert fake_ui.session_details_history[-1].users == ("mus-inst",)
...
def test_note_activity_refreshes_timer_and_details(fake_ui, config_service):
    session_manager = SessionManager(interactions=fake_ui, scheduler=fake_ui)
    first = _make_record("udr_01-mus-inst-sample_a")
    second = _make_record("udr_01-mus-inst-sample_b")
...
def test_note_activity_tracks_multiple_users(fake_ui, config_service):
    session_manager = SessionManager(interactions=fake_ui, scheduler=fake_ui)
    primary = _make_record("udr_01-mus-inst-sample_a")
    secondary = _make_record("udr_01-usr-inst-sample_b")
    ...
    latest = fake_ui.session_details_history[-1]
    assert latest.users == ("mus-inst", "usr-inst")
...
def test_note_activity_counts_increment(fake_ui, config_service):
    session_manager = SessionManager(interactions=fake_ui, scheduler=fake_ui)
    record = _make_record("udr_01-mus-inst-sample_a")
...
def test_end_session_calls_callback(fake_ui, config_service):
    ...
    session_manager = SessionManager(
        interactions=fake_ui, scheduler=fake_ui, end_session_callback=on_done
    )
...
def test_reset_timer_ignores_disabled_timeout(fake_ui, monkeypatch):
    ...
    monkeypatch.setattr("d_post.core.session.session_manager.current", lambda: stub)
