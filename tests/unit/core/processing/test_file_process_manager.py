...
from d_post.core.config import activate_device
from d_post.core.processing.file_process_manager import FileProcessManager
from d_post.core.processing.models import ProcessingCandidate, ProcessingStatus
from d_post.core.processing.stability_tracker import (
    StabilityOutcome,
...
class ErroringProcessor(DummyProcessor):
    def device_specific_processing(
        self, src_path: str, record_path: str, file_id: str, extension: str
    ) -> ProcessingOutput:
...
def test_process_item_deferred_when_preprocessing_pauses(
    manager_components, config_service, tmp_settings
):
    manager, _ = manager_components
    manager.file_processor = DeferredProcessor()

    src = tmp_settings.WATCH_DIR / "abc-inst-sample.txt"
    src.write_text("data")
...
def test_process_item_rejects_unknown_device(
    manager_components, config_service, tmp_settings
):
    manager, _ = manager_components

    src = tmp_settings.WATCH_DIR / "abc-inst-sample.bad"
    src.write_text("data")
...
def test_add_item_to_record_failure_raises(
    manager_components, config_service, tmp_settings
):
    manager, _ = manager_components
    ...
    with activate_device(config_service.devices[0]):
        with pytest.raises(RuntimeError):
            manager.add_item_to_record(
                record=None,
                src_path=str(tmp_settings.WATCH_DIR / "abc-inst-sample.txt"),
                filename_prefix="abc-inst-sample",
                extension=".txt",
                file_processor=manager.file_processor,
                notify=False,
            )
...
def test_add_item_to_record_force_paths_marked(
    manager_components, config_service, tmp_settings
):
    manager, _ = manager_components
    ...
    record_id = generate_record_id(
        prefix, dev_kadi_record_id=device.metadata.record_kadi_id
    )
...
def test_invoke_rename_flow_cancel_moves_to_manual(
    manager_components, config_service, tmp_settings
):
...
def test_process_item_ignores_internal_staging_paths(
    manager_components, tmp_settings, path_name
):
    manager, _ = manager_components
...
def test_process_item_preserves_source_extension_on_effective_path_fallback(
    ...
    monkeypatch.setattr(
        "d_post.core.processing.file_process_manager.safe_move_to_exception",
        record_move,
    )
