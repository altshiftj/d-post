...
from d_post.core.config.schema import DeviceConfig, WatcherSettings
from d_post.core.processing.device_resolver import DeviceResolver
from d_post.core.processing.file_processor_abstract import FileProbeResult
...
def _device(
    identifier: str, *, reappear: float = 0.0, retry_delay: float = 2.0
) -> DeviceConfig:
    watcher = WatcherSettings(
        reappear_window_seconds=reappear, retry_delay_seconds=retry_delay
    )
...
def test_resolve_defers_for_deferred_devices(tmp_path: Path):
    ...
    resolver = DeviceResolver(
        DummyConfigService(matching=[], deferred=[device]), DummyFactory({})
    )
...
def test_resolve_defers_for_empty_directory(tmp_path: Path):
    ...
    target.mkdir()
    resolver = DeviceResolver(
        DummyConfigService(matching=[], deferred=[]), DummyFactory({})
    )
