...
from d_post.core.config.schema import DeviceConfig, StabilityOverride, WatcherSettings
...
def test_wait_rejects_disappeared_path(tmp_path: Path):
    ...
    device = DeviceConfig(
        identifier="dev", watcher=WatcherSettings(reappear_window_seconds=0.0)
    )
...
def test_wait_times_out_when_max_wait_zero(tmp_path: Path, monkeypatch):
    ...
    device = DeviceConfig(
        ...
        watcher=WatcherSettings(
            max_wait_seconds=0.0, poll_seconds=0.0, stable_cycles=1
        ),
    )
