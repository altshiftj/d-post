...
bootstrap_mod = importlib.import_module("d_post.core.app.bootstrap")
from d_post.core.app.bootstrap import (
...
def test_build_config_service_uses_plugins(monkeypatch):
    ...
    def load_device(name):
        ...
        return type(
            "DevPluginStub",
            (),
            {"get_config": lambda self, cfg=device_configs[idx]: cfg},
        )()
...
def test_bootstrap_starts_services(monkeypatch):
    ...
    monkeypatch.setattr(
        bootstrap_mod, "start_http_server", lambda port: calls.__setitem__("prom", port)
    )
    monkeypatch.setattr(
        bootstrap_mod,
        "start_observability_server",
        lambda port: calls.__setitem__("obs", port),
    )
    monkeypatch.setattr(
        bootstrap_mod,
        "DeviceWatchdogApp",
    ...
    ui_stub = type("UIStub", (), {})()
    monkeypatch.setattr(
        bootstrap_mod, "UiInteractionAdapter", lambda ui: f"adapter:{ui}"
    )
...
def test_collect_startup_settings_defaults_sync_backend_to_noop(monkeypatch):
    """Default startup to the public-safe noop sync backend."""
    monkeypatch.setenv("PC_NAME", "pc-a")
    monkeypatch.setattr(bootstrap_mod, "get_devices_for_pc", lambda _pc: ["device"])

    settings = collect_startup_settings(load_env=False)

    assert settings.sync_backend == "noop"


def test_collect_startup_settings_rejects_unknown_sync_backend(monkeypatch):
    """Reject unsupported sync backend names from the environment."""
    monkeypatch.setenv("PC_NAME", "pc-a")
    monkeypatch.setenv("SYNC_BACKEND", "mystery")
    monkeypatch.setattr(bootstrap_mod, "get_devices_for_pc", lambda _pc: ["device"])

    with pytest.raises(StartupError, match="SYNC_BACKEND"):
        collect_startup_settings(load_env=False)


def test_bootstrap_uses_noop_sync_backend_by_default(monkeypatch):
    """Build the default noop sync manager when no private backend is requested."""
    settings = bootstrap_mod.StartupSettings(
        pc_name="pc",
        device_names=("dev1",),
        prometheus_port=9101,
    ...
    monkeypatch.setattr(
        bootstrap_mod,
        "start_http_server",
        lambda port: calls.__setitem__("prom", port),
    )
    monkeypatch.setattr(
        bootstrap_mod,
        "start_observability_server",
    ...
    ui_stub = type("UIStub", (), {})()
    monkeypatch.setattr(bootstrap_mod, "NoopSyncManager", NoopSyncStub, raising=False)
    monkeypatch.setattr(
        bootstrap_mod, "UiInteractionAdapter", lambda ui: f"adapter:{ui}"
    )
    monkeypatch.setattr(bootstrap_mod, "UiTaskScheduler", lambda ui: f"scheduler:{ui}")
    ...
    assert isinstance(context.sync_manager, NoopSyncStub)
