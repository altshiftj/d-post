...
from d_post.core.app.device_watchdog_app import DeviceWatchdogApp, QueueingEventHandler
...
def test_process_events_with_item(watchdog_app, fake_ui):
    ...
    sample_path = watch_dir / "mus-inst-sample.tif"
...
def test_process_events_surfaces_rejections(watchdog_app, fake_ui):
    watchdog_app.file_processing._rejected = [
        ("/tmp/rejected.tif", "Unsupported extension")
    ]
...
@pytest.mark.parametrize(
    ...
    ids=["keyboard_interrupt", "generic_exception"],
)
