...
def test_basic_file_processing_works(
    processing_components, tmp_settings, extension, payload
):
    ...
    expected_dir = tmp_settings.DEST_DIR / "inst" / "MUS" / "TEST-sample"
    ...
    assert (
        len(processed) == 1
    ), f"Expected exactly 1 processed file, found {len(processed)}: {processed}"
...
def test_unsupported_file_rejected(processing_components, tmp_settings):
    ...
    unsupported_path = tmp_settings.WATCH_DIR / "mus-inst-sample.pdf"
    ...
    exception_files = list(tmp_settings.EXCEPTIONS_DIR.glob("mus-inst-sample*.pdf"))
...
def test_multiple_files_same_record(processing_components, tmp_settings):
    ...
    base_dir = tmp_settings.DEST_DIR / "inst" / "MUS"
...
def test_different_user_groups_no_collision(processing_components, tmp_settings):
    ...
    for filename, user, institute in test_files:
        ...
        expected_dirs.append(
            tmp_settings.DEST_DIR
            / institute.upper()
            / user.upper()
            / f"TEST-{filename.split('-')[2]}"
        )
...
def test_mixed_file_types_processed_correctly(processing_components, tmp_settings):
    fpm, ui = processing_components
    tif_path = tmp_settings.WATCH_DIR / "mus-inst-image.tif"
    txt_path = tmp_settings.WATCH_DIR / "mus-inst-data.txt"
    ...
    expected_base = tmp_settings.DEST_DIR / "inst" / "MUS"
