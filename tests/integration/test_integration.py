...
from d_post.core.interactions.messages import InfoMessages
from d_post.core.processing.file_process_manager import FileProcessManager
from d_post.core.processing.models import ProcessingStatus
...
def _assert_processed_artifact(
    tmp_settings, institute: str, user: str, sample: str, extension: str = ".tif"
) -> Path:
    destination = (
        tmp_settings.DEST_DIR / institute.upper() / user.upper() / f"TEST-{sample}"
    )
    ...
    assert (
        len(matches) == 1
    ), f"Expected exactly one {extension} artefact in {destination}, found {matches}"
    return matches[0]
...
def real_processing_app(config_service, tmp_settings, monkeypatch):
    ...
    monkeypatch.setattr(
        "d_post.core.app.device_watchdog_app.Observer",
        lambda: observer_stub,
    )
    app = DeviceWatchdogApp(
        ui=cast(
            Any, ui
        ),  # HeadlessUI implements UserInteractionPort, which is compatible with UserInterface
        sync_manager=sync,
...
def test_happy_path(real_processing_app, tmp_settings):
    ...
    artefact = _assert_processed_artifact(tmp_settings, "inst", "mus", "sample")
...
def test_event_queue_processes_pending_items(real_processing_app, tmp_settings):
    ...
    _assert_processed_artifact(tmp_settings, "inst", "mus", "queued")
...
def test_invalid_inputs_route_to_expected_bucket(
    real_processing_app, tmp_settings, filename, expected_dir, expected_message
):
    ...
    bucket = (
        tmp_settings.EXCEPTIONS_DIR
        if expected_dir == "exceptions"
        else tmp_settings.RENAME_DIR
    )
...
def test_interactive_rename_loop_success(real_processing_app, tmp_settings):
    real_processing_app.ui.rename_inputs.append(
        {"name": "mus", "institute": "inst", "sample_ID": "sample"}
    )
...
def test_session_end_flushes_on_done(real_processing_app, tmp_settings):
    ...
    assert (
        sync_mgr is not None and sync_mgr.synced_records
    ), "Expected sync manager to be invoked"
...
def test_rapid_file_arrival_same_record(real_processing_app, tmp_settings):
    ...
    assert (
        len(all_tif_files) == num_files
    ), f"Expected {num_files} files, found {len(all_tif_files)}"
...
def test_multiple_files_same_record(real_processing_app, tmp_settings):
    ...
    for i in range(num_files):
        dest = tmp_settings.DEST_DIR / "inst" / "USR" / f"TEST-threadsafe{i}"
