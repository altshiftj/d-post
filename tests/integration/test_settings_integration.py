...
from d_post.core.config import activate_device, current, init_config, reset_service
from d_post.core.config.schema import (
    DeviceConfig,
    DeviceFileSelectors,
    DeviceMetadata,
    NamingSettings,
...
def config(tmp_path):
    ...
    pc = PCConfig(
        ...
        watcher=WatcherSettings(
            poll_seconds=0.5, max_wait_seconds=10.0, stable_cycles=2
        ),
    ...
    device_a = DeviceConfig(
        ...
        metadata=DeviceMetadata(
            device_abbr="A", default_record_description="desc A", user_kadi_id="a-user"
        ),
        files=DeviceFileSelectors(allowed_extensions=frozenset({".tiff", ".tif"})),
        session=SessionSettings(timeout_seconds=120),
        watcher=WatcherSettings(
            poll_seconds=0.1, max_wait_seconds=5.0, stable_cycles=1
        ),
    )
    ...
    device_b = DeviceConfig(
        identifier="device_b",
        metadata=DeviceMetadata(
            device_abbr="B", default_record_description="desc B", user_kadi_id="b-user"
        ),
        files=DeviceFileSelectors(allowed_extensions=frozenset({".txt", ".csv"})),
        session=SessionSettings(timeout_seconds=300),
        watcher=WatcherSettings(
            poll_seconds=0.2, max_wait_seconds=7.0, stable_cycles=2
        ),
    )
