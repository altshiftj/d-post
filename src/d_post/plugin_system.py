...
_PLUGIN_NAMESPACE = "d_post"
DEVICE_ENTRYPOINT_GROUP = "d_post.device_plugins"
PC_ENTRYPOINT_GROUP = "d_post.pc_plugins"
    ...
    def create(self, name: str) -> DevicePlugin:
        try:
            ...
            raise RuntimeError(
                ...
                "Install a package that provides this plugin or check casing."
            ) from exc
        plugin = factory()
...
class PCPluginRegistry(_BaseRegistry):
    ...
    def create(self, name: str) -> PCPlugin:
        ...
        if not isinstance(plugin, PCPlugin):
            raise TypeError(
                f"Factory for PC plugin '{name}' returned {type(plugin)!r}, expected PCPlugin"
            )
        return plugin
...
class PluginLoader:
    ...
    def _load_builtin_plugins(self) -> None:
        packages = (
            ("d_post.device_plugins", DEVICE_ENTRYPOINT_GROUP),
            ("d_post.pc_plugins", PC_ENTRYPOINT_GROUP),
        )
    ...
    def _lazy_load_builtin(self, group: str, name: str) -> bool:
        ...
        if group == DEVICE_ENTRYPOINT_GROUP:
            module_name = f"d_post.device_plugins.{name}.plugin"
        elif group == PC_ENTRYPOINT_GROUP:
            module_name = f"d_post.pc_plugins.{name}.plugin"
        else:
            return False
...
def get_plugin_loader() -> PluginLoader:
    ...
    if _PLUGIN_LOADER_SINGLETON is None:
        _PLUGIN_LOADER_SINGLETON = PluginLoader(
            load_entrypoints=False, load_builtins=False
        )
